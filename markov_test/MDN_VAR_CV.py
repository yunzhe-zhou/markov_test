    ...
    if sim_type=="var":
        ...
        for k in range(T0):
            if k>=lag:
                ...
                cov=nstd*np.eye(x_dims)*add_basis.T
...
def MDN_learning1(z_train,x_train,z_test0,hidden_units,k_mixt,M,num_iter,lr):
    np.random.seed(2021)
    random.seed(2021)
    tf.random.set_seed(2021)
...
def MDN_learning2(z_train,x_train,z_test0,hidden_units,k_mixt,M,num_iter,x_sim,lr):
    np.random.seed(2021)
    random.seed(2021)
    tf.random.set_seed(2021)
